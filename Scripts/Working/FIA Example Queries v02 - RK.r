...
# Now load some PEcAn functions, written by R. Kooper, M. Dietze, et al. If you have PEcAn you can just load the library, but if not, download PSQL_utils.R from the macrosystems-FIA folder (currently https://duke.box.com/macrosystems-FIA), and point to it here:
...
#   )
  ...
  dbsettings = list(
    ...
    password = "",                # PSQL password
    dbname   = "fia5data_pecan",  # PSQL database name
    host     = "localhost",       # PSQL server address (don't change unless server is remote)
    driver   = 'PostgreSQL',      # DB driver (shouldn't need to change)
    write    = FALSE              # Whether to open connection with write access. 
  )
...
# Set species of interest here. Obtain SPCD codes from Appendix F of the FIA manual. 
  spcd = c(131, 802)
